...
// Mirrors BracketContext state but is completely separate.
// Also overrides BracketContext.Provider so all existing components
// (GroupCard, BracketSidebar, BracketPage etc.) automatically use
// prediction state when rendered inside PredictionProvider.
export function PredictionProvider({ children }) {
  ...
  const selectGroupTeam = useCallback((gId, team) => {
    ...
    if (isW) { setGW((p) => { const n = { ...p }; delete n[gId]; return n; }); resetBracket(); return; }
    if (isR) { setGR((p) => { const n = { ...p }; delete n[gId]; return n; }); resetBracket(); return; }
    if (!gW[gId]) { setGW((p) => ({ ...p, [gId]: team })); resetBracket(); return; }
    if (!gR[gId] && gW[gId]?.n !== team.n) { setGR((p) => ({ ...p, [gId]: team })); resetBracket(); }
  }, [gW, gR]); // eslint-disable-line

  const pickR32 = useCallback((idx, team) => { setR32((p) => ({ ...p, [idx]: team })); clearFromR32(idx); }, [clearFromR32]);
  const pickR16 = useCallback((idx, team) => { setR16((p) => ({ ...p, [idx]: team })); clearFromR16(idx); }, [clearFromR16]);
  const pickQF  = useCallback((idx, team) => { setQF((p)  => ({ ...p, [idx]: team })); clearFromQF(idx);  }, [clearFromQF]);
  const pickSF  = useCallback((idx, team) => { setSF((p)  => ({ ...p, [idx]: team })); setFin(null); setThirdPlace(null); }, []);
  const pickFinal      = useCallback((team) => setFin(team), []);
  const pickThirdPlace = useCallback((team) => setThirdPlace(team), []);

  function resetBracket() { setR32({}); setR16({}); setQF({}); setSF({}); setFin(null); setThirdPlace(null); }
  const resetAll = useCallback(() => { setGW({}); setGR({}); resetBracket(); }, []);
  ...
  return (
    <PredictionCtx.Provider value={value}>
      {/* Override BracketContext so GroupCard, BracketSidebar, etc. all use prediction state */}
